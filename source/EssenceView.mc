...
class EssenceView extends WatchUi.WatchFace {
      ...
      if (hiddenByGraph) {
        ...
        view.setText(
          WatchUi.loadResource(
            fieldCatalog[fieldLayout[i]["data"]]["label"]
          ) as String
        );
      }
    }
  ...
  function drawData(dc) {
    ...
    for (var i = 0; i < fieldLayout.size(); i = i + 1) {
      ...
      if (hiddenByGraph) {
        ...
        var labelView = View.findDrawableById(
          fieldLayout[i]["id"] + "Label"
        ) as Text;
  ...
  function drawIcons(dc) {
    ...
    if (settings.doNotDisturb)    { icons += (127).toChar().toString(); }
    if (settings.phoneConnected)  { icons += "V"; }
    if (settings.alarmCount > 0)  { icons += "R"; }
  ...
  function drawGraph(dc) {
    ...
    if (sampleData == null || curMin == null || curMax == null) { return; }
    if (curMin == 0 || curMax == 0 || curMax <= curMin)         { return; }
      ...
      while (!finished && secsBin < binWidthSecs) {
        ...
        if (graphValue != null) {
          if (graphBinMax == 0) {
            ...
            if (graphValue > graphBinMax) { graphBinMax = graphValue; }
            if (graphValue < graphBinMin) { graphBinMin = graphValue; }
          }
        }
  ...
  function defineBoundingBoxes(dc) {
    ...
    var bboxTop = [[col, 0], [col * 2, row]];

    var bboxUpperLeft   = [[0,       row], [col,     row * 2]];
    var bboxUpperCenter = [[col,     row], [col * 2, row * 2]];
    var bboxUpperRight  = [[col * 2, row], [dw,      row * 2]];

    var bboxLowerLeft   = [[0,       rowB], [col,     rowB + row]];
    var bboxLowerCenter = [[col,     rowB], [col * 2, rowB + row]];
    var bboxLowerRight  = [[col * 2, rowB], [dw,      rowB + row]];

    var bboxBottom = [[col, dh - row], [col * 2, dh]];
    ...
    if (graphSize == 1) {
      bboxLowerLeft   = [[0, 0], [0, 0]];
      bboxLowerCenter = [[0, rowB], [dw, rowB + row]];
      bboxLowerRight  = [[0, 0], [0, 0]];
    }

    boundingBoxes = [
      { "id" => "FieldTop",         "bounds" => bboxTop,         "value" => "", "complicationId" => Complications.COMPLICATION_TYPE_CURRENT_WEATHER   },
      { "id" => "FieldUpperLeft",   "bounds" => bboxUpperLeft,   "value" => "", "complicationId" => Complications.COMPLICATION_TYPE_CALENDAR_EVENTS    },
      { "id" => "FieldUpperCenter", "bounds" => bboxUpperCenter, "value" => "", "complicationId" => Complications.COMPLICATION_TYPE_NOTIFICATION_COUNT },
      { "id" => "FieldUpperRight",  "bounds" => bboxUpperRight,  "value" => "", "complicationId" => Complications.COMPLICATION_TYPE_SUNRISE            },
      { "id" => "FieldLowerLeft",   "bounds" => bboxLowerLeft,   "value" => "", "complicationId" => Complications.COMPLICATION_TYPE_ALTITUDE           },
      { "id" => "FieldLowerCenter", "bounds" => bboxLowerCenter, "value" => "", "complicationId" => Complications.COMPLICATION_TYPE_HEART_RATE         },
      { "id" => "FieldLowerRight",  "bounds" => bboxLowerRight,  "value" => "", "complicationId" => Complications.COMPLICATION_TYPE_SEA_LEVEL_PRESSURE },
      { "id" => "FieldBottom",      "bounds" => bboxBottom,      "value" => "", "complicationId" => Complications.COMPLICATION_TYPE_BATTERY            },
    ];
  }
  ...
  function getWeather() {
    if (Toybox has :Weather) {
      ...
      if (data == null) { return "--"; }
      return (data.lowTemperature  + 0.5).toNumber().toString() + "/" +
  ...
  function getCalendar() {
    if (Toybox has :Complications) {
      ...
      if (comp.value != null) { return comp.value; }
    }
    return "--";
  }
  ...
  function getSunEvent() {
    ...
    if (positionInfo.position == null) { return fallback; }

    if (!(Toybox has :Weather)) { return fallback; }
    ...
    if (sunrise == null || sunset == null) { return fallback; }
  ...
  function getDate() {
    ...
    return Lang.format("$1$, $2$", [days[clockTime.day_of_week], medium.day]).toLower();
  }
  ...
  function getBattery() {
    ...
    if (Toybox has :Complications) {
      ...
      if (comp.value != null) { data = comp.value; }
    }
  ...
  function getBatteryDays() {
    if (Toybox has :System) {
      if (Toybox.System.getSystemStats() has :batteryInDays) {
        ...
        if (data != null) { return data.format("%d"); }
      }
    }
    return "--";
  }
  ...
  function getAltimeter() {
    ...
    if (data == null) {
      ...
      if (sample.data != null) { data = sample.data; } else { return "--"; }
    }
  ...
  function getBarometer() {
    ...
    if (data == null && (Toybox has :SensorHistory)) {
      if (Toybox.SensorHistory has :getPressureHistory) {
        ...
        var sample   = iterator.next();
        if (sample != null) { data = sample.data; } else { return "--"; }
      }
    }
    if (data == null) { return "--"; }
  ...
  function drawBoundingBoxes(dc) {
    ...
    for (var i = 0; i < boundingBoxes.size(); i = i + 1) {
      ...
      dc.drawText(cx, cy - dc.getFontHeight(font), font,
        boundingBoxes[i]["id"], Graphics.TEXT_JUSTIFY_CENTER);
      dc.drawText(cx, cy, font,
        boundingBoxes[i]["value"], Graphics.TEXT_JUSTIFY_CENTER);
    }
  }
}
